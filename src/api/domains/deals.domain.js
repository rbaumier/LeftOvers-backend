...
module.exports = (db, defaultCallback) => {
  ...
  return {
    findAll(geolocation, user_id, f) {
      ...
      db.preferences.findOne({ user_id }, (err, preferences = {}) => {
        if (err) return f(err);
        db.run(`SELECT * FROM dealers WHERE ST_DWithin(geolocation, Geography(ST_MakePoint(${geolocation})), $1)`, [preferences.radius || defaultRadius], (err, dealers) => {
          if (err) return f(err);
